...
The influential desirable difficulty principle suggests that making learning harder not easier, such as having students take a test over information previously studied, can have noticeable and lasting impacts on student achievement [@Bjork2011; see @Sotola2020 for a recent meta-analysis]. Recently, the concept of desirable difficulties has been extended to include subtle perceptual manipulations that are difficult to encode [e.g., atypical fonts, blurring, handwritten cursive; @Diemand-Yauan2011; @Yue2012; @Geller2018]. One such perceptual disfluency manipulation garnering increased attention from news outlets (NPR and Washington Post) and researchers alike is the Sans Forgetica typeface. Sans Forgetica is a typeface developed by a team of psychologists, graphic designers, and marketers, consisting of intermittent gaps and black-slanted letters [@Earp2017]. The perceptual characteristics of Sans Forgetica are purported to stave off forgetting and enhance learning. However, as Carl Sagan once said, "Extradionary claims require extradionary evidence.” []
...
Performance was examined with d', a memory sensitivity measure derived from signal detection theory [@Macmillan2005]. Hits or false alarms at ceiling or floor were changed to .99 or .01. Hits and false alarms along with sensitivity (d') can be seen in Figure 1. Per our pre-registation, sensitivity (d') as analyzed with a 2 (Typeface: Arial vs. Sans Forgetica ) x 2 (Testing Expectancy: High vs. Low) Mixed analysis of variance (ANOVA). Participants that were told about a memory test performed better than those not told about a memory test,*M* ~diff~ = 0.161,*F*(1, 229) = 4.11, $\eta_{g}^{2}$ = .014, p = .044. Individuals were better at discriminating target words presented in Sans Forgetica than Arial,*M* ~diff~ = 0.125  *F*(1, 229) = 10.73, $\eta_{g}^{2}$ =.010, *p* = .001. This was qualified by an interaction between Test Expectancy and Typeface, *F*(1, 229) = 4.34, $\eta_{g}^{2}$ = .004, *p* = .038. Simple effects showed that individuals in the low expectancy group showed better recognition memory for words presented in Sans Forgetica font compared to Arial, *F*(1, 229) = 14.297, *p* < .001. In the high test expectancy group, there was  d` differences between the two typefaces, *F*(1, 229) = 0.716, *p* = .398, BF~O1~ = . 


### JOLs

Seven participants were removed for not providing JOls to each typeface. Using the same model as above, JOLs were higher when testing expectancy was lower, *F*(1,221) = 16.01, $\eta_{g}^{2}$ = .065, *p* < .001. JOLs were lower for Sans Forgetica than Arial, *M* ~diff~ = 4.0, *F*(1,221) = 27.05, $\eta_{g}^{2}$ = .004, *p* < .001. There was no interaction between Texting Expextancy and Typeface, *F*(1,221) = 0.13, $\eta_{g}^{2}$ < .001, *P* = .715. 
...
jols<-jol_high_low %>% janitor::clean_names(.) %>% mutate(date=as.Date(utc_date)) %>%  dplyr::filter(date=="08/06/2020" | date=="0009/07/2020"|date=="0010/07/2020" | date=="09/06/2020",  zone_type=="response_slider_endValue" | zone_type=="response_text_entry")
...
jols1<- jols %>% 
  ...
  mutate(cond=rep(1:2, 231), font=ifelse(cond==1, "SF", "A")) %>%
  select(-cond)%>%
  drop_na()
...
Although not pre-registered, outliers 2.5 SD above the mean per condition for each participant were removed. The outlier procedure removed ~3 % of the data. Given reactions times are notoriously positively skewed, we also log transformed the data (see Fig.1C for reaction time data). Testing Expectancy did not influence reading times, *F*(1,229) = 1.97, $\eta_{g}^{2}$ = .008, *p* =  .162, BF. Typeface did influence reading times. Response latencies were overall slower for Sans Forgetica than Arial, *F*(1,229) = 30.91, $\eta_{g}^{2}$ = .001, *p* < .001. There was no interaction between Testing Expextancy and Typeface, *F*(1,229) = 1.10, $\eta_{g}^{2}$ < .001, *p* = .296.
...
rt1<- rt %>% 
  ...
  dplyr::mutate(sdabove = mean(reaction_time, na.rm=TRUE) +  2.5*sd(reaction_time, na.rm=TRUE)) %>%
    dplyr::filter(reaction_time < sdabove) %>% 
  dplyr::summarise(mean_rt=median(reaction_time), log_rt=log(reaction_time))



fig_rt <- ggplot(rt1,aes(x=condition,y=log_rt,fill=condition))+ facet_grid(~testexpt) + 
  ...
  geom_boxplot(aes(x = condition, y = log_rt),outlier.shape = NA,
               alpha = 0.3, width = .1, colour = "BLACK") +
  theme_cowplot() +
  scale_colour_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2") +
  labs(y = "Reading Times (ms)", x = "Typeface") + theme(legend.position = "none")
...
```{r}
a1 <- aov_ez("participant_private_id", "log_rt", rt1, 
...
With low testing expectancy, performance was better when words were presented in Sans Forgetica compared to Arial (*M* ~diff~ = 0.14), *t*(115) = -2.363, *SE* =  0.046, *p* = .020, *d* = -0.219.

### Reaction Times

To account for outliers, we removed RTs 3 SDs above and below  each participant's mean. Because the raw data showed significant departures from normality (see Fig2C), a Wilcoxon signed-rank test was computed to assess whether reading times for Sans Forgetica were longer than for  Arial. Self-paced reading times for Sans Forgetica were longer than reading times for Arial, (*M* ~diff~ = -167), W = 2108, *SE* =  236, *p* < .001, *r* = -.379.

### JOLs

Similar to reading times, a Wilcoxon signed-rank test was computed to assess whether JOLs for Sans Forgetica were longer than for  Arial. Sans Forgetica JOLs were lower than Arial JOLs, *M* ~diff~ = -10.00 , W = 240, *SE* = 1.61, *p* < .001, *r* = -0.557). 
...
```{r, echo=TURE, fig.cap="Accuracy on cued recall test", fig.height=6, fig.width=8, message=FALSE, warning=FALSE, results="asis"}}

library(dabestr)

cued_fig <- read.csv(here("cue_recall", "summary_data", "cued", "dd_acc.csv"))
...
fig1a <- ggplot(two.group.paired,aes(x=Typeface,y=accuracy,fill=Typeface))+
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .4,adjust=4)+
  ...
  scale_colour_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2") +
  labs(y = "Cued Recall Accuracy", x = "Typeface") + theme(legend.position = "none")
...
fig1b <- ggplot(two.group.pairedJOL,aes(x=Typeface,y=JOLs,fill=Typeface))+
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .4,adjust=4)+
  geom_point(position=position_jitter(width = .15),size = 1, alpha = 0.4) + 
...
cued_rt <- read.csv(here("cue_recall", "summary_data", "rts", "cued_rt_summary.csv"))
...
fig1c <- ggplot(two.group.pairedrt,aes(x=Typeface,y=JOLs,fill=Typeface))+
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .4,adjust=4)+
  geom_point(position=position_jitter(width = .15),size = 1, alpha = 0.4) + 
  geom_boxplot(aes(x = Typeface, y = JOLs),outlier.shape = NA,
               alpha = 0.3, width = .1, colour = "BLACK") +
  ...
  scale_colour_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2") +
  labs(y = "Reaction times (ms)", x = "Typeface") + theme(legend.position = "none")
...
```{r, echo=TURE, fig.cap="Judgements of Learning", fig.height=6, fig.width=8, message=FALSE, warning=FALSE, results="asis"}}

library(dabestr)

cued_jol <- read.csv(here("cue_recall", "summary_data", "cued", "dd_acc.csv"))
...
two.group.paired <- 
  ...
  dabest(Typeface, accuracy, 
         idx = c("Arial", "Sans Forgetica"), 
         paired = TRUE, id.col = participant_private_id )

fig_cued<- two.group.paired %>% 
  cohens_d() %>% 
  plot(rawplot.ylabel="Cued Recall Accuracy")
