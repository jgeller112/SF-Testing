...
Seven participants were removed for either not providing JOls to each typeface, or only providing one response. Using the same model as above, JOLs were higher when testing expectancy was lower, *F*(1,221) = 16.01, $\eta_{g}^{2}$ = .065, *p* < .001. JOLs were lower for Sans Forgetica (*M* = 57.5) compared to Arial (*M* = 61.5), *M* ~diff~ = 4.0, *F*(1,221) = 27.05, $\eta_{g}^{2}$ = .004, *p* < .001. There was no interaction between Testing Expectancy and Typeface, *F*(1,221) = 0.13, $\eta_{g}^{2}$ < .001, *P* = .715.There was little evidence for an interaction, BF~01~ = 7.28. 
...
jols1<- jols %>%
  ...
  tidyr::drop_na() %>% 
  dplyr::mutate(testexpt=ifelse(testexpt=="low", "Low Test Expectancy", "High Test Expectancy"), font=ifelse(font=="A", "Arial", "Sans Forgetica"))
  


figjol <- ggplot(jols1,aes(x=font,y=response,fill=font))+ facet_grid(~testexpt) + 
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .4,adjust=4)+
  geom_point(position=position_jitter(width = .15),size = 1, alpha = 0.2) + 
...
```{r, fig.align="center", fig.cap="Raincloud plots (Allen et al., 2019) depicting raw data (dots), box plots, and half violin kernel desntiy plots.A.Memory sensitivity (d') as a function of Typeface and Testing Expectancy. B. Judgements of Learning as a function of Typeface and Test Expectany. C. Study times (log transformed) as a function of Typeface and Test Expextancy. Raincloud plots (Allen et al., 2019) depicting raw data (dots), box plots, and half violin kernelViolin plots represent the kernal density of avearge accuracy (black dots) with the mean (white dot)", fig.height=6, fig.width=8, message=FALSE, warning=FALSE, results="asis"}
...
We examined cued recall accuracy, JOLs, and reading times to Typefaces (Sans Forgetica vs. Arial) with a paired *t*-test. 
...
With low testing expectancy, performance was better when words were presented in Sans Forgetica (*M* = .47, *SD* = .26) compared to Arial (*M* = .42, *SD* = .27), *M*~diff~ = 0.05, *t*(115) = 2.363, *SE* =  0.046, *p* = .020, 95 CI% [0.008, 0.090], *d*~avg~ = 0.18. See fig 2a. 
...
Looking at particpants JOLs to each Typeface, Partcipants' JOLs were lower for Sans Forgetica (*M* = 65.83, *SD* = 32.7) compared to Arial (*M* = 70.84. *SD* = 32.6), *M* ~diff~ = -5.02, *t*(108) = -3.12, *SE* =  1.61, 95 CI% [0.030, 0.114], *p* = .002, *d*~avg~ = 0.15. See fig 2a.
