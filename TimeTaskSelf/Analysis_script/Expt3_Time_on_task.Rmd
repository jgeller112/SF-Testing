...
fig3a <- ggplot(recall_all1,aes(x=Typeface,y=acc,fill=Typeface))+ 
  ...
  geom_pointrange(data=sfgen_wsci, aes(y=acc, ymin=acc-ci, ymax=acc+ci), size=.8, color="darkred")+ 
...
fig3b <- ggplot(jol_rename,aes(x=Typeface,y=round(jols,2),fill=Typeface))+ 
  ...
  geom_pointrange2(data=sfgenjol_wsci, aes(y=jols, ymin=jols-ci, ymax=jols+ci), size=.8, color="darkred")+ 
  theme_cowplot() +
  scale_colour_brewer(palette = "Dark2")+
  scale_fill_brewer(palette = "Dark2") +
